...
params = struct('a',1.6e-2,'n',4,'beta',10,...
    ...
    'u_input',[0,-0.005*9.8],'u_output',[0,1;0,0]);
...
% fix leftmost and rightmost nodes
ind_fix = [1,5];
...
optims = struct('alpha',5e-3,'beta1',0.9,'beta2',0.999,...
    'k_conv',1e-8,'u_conv',1e-2,'MaxIter',10000);
...
for i = 1:size(bonds,1)
    plot([DT.Points(bonds(i,1),1),DT.Points(bonds(i,2),1)],[DT.Points(bonds(i,1),2),DT.Points(bonds(i,2),2)],'k-',...
        'LineWidth',1);
    hold on;
    save_data = [save_data,[[DT.Points(bonds(i,1),1);DT.Points(bonds(i,2),1)],[DT.Points(bonds(i,1),2);DT.Points(bonds(i,2),2)]]];
end
...
while 1
    % Get gradients
    [results.fHist(iter),u,eori,F_backward] = Spring2D_CE(k0,F,params,optims,'1');
    [~,uadj,eadj,~] = Spring2D_CE(k0,F_backward,params,optims,'1');
    sgCurr = eadj.*eori;
    ...
    % Convergence
    if Check_conv_CE(results.fHist,results.k_error,iter,optims)
        break;
    else
        iter = iter + 1;
    end
